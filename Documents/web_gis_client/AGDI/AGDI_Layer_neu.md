...
# Beschreibung zum Ablauf "Ändern eines Layers"

![Alt-Text](https://github.com/bjsvwcur/DockTick_Funktionale_Einheiten/blob/master/web_gis_client/AGDI/Ablauf_Layererfassung_aendern_erweitern.jpg)

**Änderung mit Strukturanpassung**

Einer in der Produktions-DB der geodb bestehenden Tabelle wird ein neues Attribut hinzugefügt oder ein bestehendes gelöscht.

**Änderung ohne Strukturanpassung**

Änderungen an den SQL-Queries, welche aber keine Auswirkungen auf die bestehenden Tabellenstrukturen hat.

**Anpassung AGDI**

Änderung am Layer, welche lediglich im AGDI vorgenommen werden muss.
...
Tracker 	Layer Web GIS Client
neuer Redminestatus: 	neu
Änderungstyp 	"Änderung mit Strukturanpassung" oder "Änderung ohne Strukturanpassung" oder "Anpassung AGDI"
...
Je nach des Tickets/Auftrags gibt es unterschiedliche weitere Vorgehensweisen. Handelt es sich um einen eine Änderung mit oder ohne Strukturanpassung so muss ein neuer GRETL-Job erstellt respektive ein bestehender GRETL-Job angepasst werden. Handelt es sich hingegen um eine Anpassung im AGDI so muss der entsprechende Layer im AGDI angepasst respektive erfasst werden.
...
Je nach Typ wird im Anschluss an GRETL-Job erstellen/anpassen ein DDL-Skript respektive ein Modell erstellt (Änderung mit Strukturanpassung) oder es wird sogleich ein Pullrequest (Änderung ohne Strukturanpassung) erstellt.

**DDL-/Modell-Skript erstellen**

Existiert die Tabelle resp. das Schema noch nicht auf der Publikattions-DB, so muss ein entsprechendes SQL-Skript erstellt werden, welches das Schema resp. die Tabelle erstellt. Dies ist jedoch nicht der Fall, wenn es sich dabei um ein Modell handelt oder die Tabelle bereits in der Publikations-DB vorhanden ist, aber angepasst werden muss.

Existiert die Tabelle bereits so muss ein SQL-Skript erstellt werden, welches die Änderungen an der Tabellenstruktur vornimmt.
...
Sollte alles in Ordnung sein, so bleibt das Ticket bei der GDI und erhält den Status "reviewed GDI".
neuer Redminestatus: 	reviewed GDI
...
**GRETL-Job im Branch ausführen**

Nach dem die Tabellen angepasst wurden, kann der GRETL-Job im Branch ausgeführt werden und so die Tabellen abfüllen. Sollte dieser fehlschlagen, so ist dies im Redmine-Ticket zu vermerken, das Ticket wird dem Auftragnehmer zurückgewiesen und der Redmine-Status auf "in Bearbeitung" gesetzt.
neuer Redminestatus: 	in Bearbeitung
Zugewiesen an: 	Auftragnehmer

Sollte der GRETL-Job einwandfrei durchlaufen, so erhält das Ticket den Redmine-Status "vorbereitet Integrations-DB" und wird dem Auftragnehmer zugwiesen.
neuer Redminestatus: 	vorbereitet Integrations-DB
Zugewiesen an: 	Auftragnehmer
...
Je nach Typ muss, bevor das ganze ausgerollt werden kann, noch der Branch in den Master gemergt werden.

**Merge von Pullrequest**

Wenn beim Review alles in Ordnung war, kann der Branch in den Master gemerged werden. Im Anschluss an den Merge soll der Branch gelöscht werden.

**GRETL-Job im Master ausführen (Integration)**

Um die Daten definitiv in die Tabellen zu spielen muss der GRETL-Job in der Integrationsumgebung ausgeführt werden. Sollte dieser fehlschlagen, so ist dies im Redmine-Ticket zu vermerken, das Ticket wird dem Auftragnehmer zurückgewiesen und der Redmine-Status auf "in Bearbeitung" gesetzt.
neuer Redminestatus: 	in Bearbeitung
Zugewiesen an: 	Auftragnehmer

Sollte der GRETL-Job einwandfrei durchlaufen, so erhält das Ticket den Redmine-Status "merged in master".
neuer Redminestatus: 	merged in master
